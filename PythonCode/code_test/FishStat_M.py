def day_stat(day,fishs):
    ...
    print('%s数量小计%d,金额小计%.2f元'%(day,nums,amount))


def allday_stat(fishs):
    '''
    统计所有鱼，并保存到统计字典里
    :param fishs: 为两层  字典参数
    :return:
    ...
    sub_records={}
    stat_record={}
    for day,day_record in fishs.items():
        for name1,sub_records in day_record.items():
            if name1 in stat_record:
                stat_record[name1][0]+=sub_records[0]
                stat_record[name1][1]+=sub_records[0]*sub_records[1]
            else:
                stat_record[name1]=[sub_records[0],sub_records[0]*sub_records[1]]
    #print(stat_record)
    return stat_record
def MaxNum(stat_record):
    maxs=['',0,'',0,0,0]
